function status = find_classes(pasym_gi_T, pmar_gi_T, tlik, Params)
    ...
    % asym
    asymgenes = find(pasym_gi_T > p_thresh_asym); %find genes with prob(a_i | g_i, T) > p_thresh_asym
    ...
    status(asymgenes(group==1)) = 1; %mark status in output vector
    status(asymgenes(group==2)) = -1;
    
    % sym
    psym_gi_T = pmar_gi_T(:,tlik);
    symgenes = find(psym_gi_T > p_thresh_sym); %find genes with prob(b_i | g_i, T) > p_thresh_sym
    ...
    % markers for daughters
    daught = setdiff(1:3,tlik);
    daught1 = daught(1); % code: +3
    daught2 = daught(2); % code: -3
    markergenes1 = find(pmar_gi_T(:,daught1) > p_thresh_sym); %find genes with prob(b_i | g_i, T) > p_thresh_sym
    markergenes2 = find(pmar_gi_T(:,daught2) > p_thresh_sym); %find genes with prob(b_i | g_i, T) > p_thresh_sym
