function [IA, IAB, IABCsame, IABC_Amin, IABC_Bmin, IABC_Cmin, IA_BC_Amax, IA_BC_Bmax, IA_BC_Cmax] = calculate_integrals_both_no_parallel_test4(log2tfdata, idxs, iunique, P, M, S, cutoff)
...
% %         F1 = ((1./sqrt(2*pi*S.^2)).^N1);
% %         for ii=1:N1
% %             F1 = F1.*exp(-((M-xk1(ii)*ones(size(M))).^2)./(2*S.^2));
% %         end
        F1 = -N1*logS/2; 
        ...
        for k=(j+1):ncells %integrals over 2 cell types (to use in integrals 3,4 and 5)
            ...
            xk12 = [xk1 xk2];
% %             F2 = ((1./sqrt(2*pi*S.^2)).^N12);%%%%%%%%% old way
% %             for ii=1:N12%%%%%%%%%%
% %                 F2 = F2.*exp(-((M-xk12(ii)*ones(size(M))).^2)./(2*S.^2));%%%%%%5
% %             end%%%%%%%
            F2 = -N12*logS/2; 
